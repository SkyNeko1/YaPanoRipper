...
### Если через `file://` не работает CORS

У некоторых браузеров могут быть строгие правила при открытии HTML через `file://` протокол. В этом случае подними простой локальный сервер:
